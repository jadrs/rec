...
if __name__ == '__main__':
    ...
    transform = tr.Compose([
        ...
        tr.SquarePad(),
        tr.Resize(size=(320, 320)),
        #tr.RandomHorizontalFlip(),
        tr.RandomAffine(degrees=5, translate=(0.1, 0.1), scale=(0.9, 1.1)),
        #tr.ColorJitter(brightness=0.5, saturation=0.5, contrast=0.5),
        tr.NormalizeBoxCoords()
    ])

    idx = 14982

    # ds_train = RefCOCO('train', transform=transform)#, noun_chunks_file='cache/etags_train2014.json')
    # ds_val = RefCLEF('val', with_mask_bbox=True, transform=transform)#, noun_chunks_file='cache/etags_train2014.json')
    # ds_test = RefCOCOg('test', transform=transform)#, noun_chunks_file='cache/etags_train2014.json')

    ds_val = RegionDescriptionsVisualGnome('/media/jrg/DATA/datasets/VisualGnome', transform=transform)

    # classes = ds_train.classes_from_tags(min_freq=2)
    # ds_train.labels_from_tags(classes)
    # ds_val.labels_from_tags(classes)
    ...
    for i in range(50):
        ...
        doc = nlp(expr)
        print(doc)
        print([(w, w.pos_) for w in doc])
        print([nc for nc in doc.noun_chunks])
        print([nc.root.dep_ for nc in doc.noun_chunks])
        print([doc[nc.start-1:nc.end] for nc in doc.noun_chunks if nc.start > 0 and doc[nc.start-1].pos_ == 'ADP'])
        print()
    ...
    # from transforms import undo_box_transforms
    # if 'tr_param' in sample:
    #     bbox = undo_box_transforms(bbox, sample['tr_param'])
    ...
    img = (img * 255).byte()
    mask = (mask * 255).byte()
    # img = (sample['mask'].repeat(3,1,1) * 255).byte()
    visu = draw_bounding_boxes(img, bbox, colors=[(255,0,0),])
    plt.figure(1)
    plt.imshow(visu.permute(1, 2, 0))
    plt.figure(2)
    plt.imshow(mask[0])
    plt.figure(3)
    plt.imshow(mask_bbox[0])
    plt.show()

    # loader = torch.utils.data.DataLoader(
    #         ds,#RefCOCO('val', transform=transform),
    #         batch_size=4,
    #         shuffle=False,
    #         num_workers=0,
    #         pin_memory=False,
    ...
    # sys.path.append('refer')
    # from refer import REFER
    # refer = REFER('refer/data', 'refcoco', 'unc')
    # image_ids = []
    # for split in ('val', 'testA', 'testB'):
    #     ref_ids = sorted(refer.getRefIds(split=split))
    #     image_ids += [refer.Refs[rid]['image_id'] for rid in ref_ids]
    # image_ids = sorted(image_ids)
    # with open('./refcoco_valtest_ids.txt', 'w') as fh:
